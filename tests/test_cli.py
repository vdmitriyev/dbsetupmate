...
def test_a_declined_drop_changes_nothing(runner, server):
    ...
    result = runner.invoke(cli.app, ["db", "drop-db", "--db-name", "course_db_01"], input="n\n")
...
def test_a_confirmed_drop_goes_ahead(runner, server):
    ...
    result = runner.invoke(cli.app, ["db", "drop-db", "--db-name", "course_db_01"], input="y\n")
...
def test_yes_skips_the_prompt(runner, server):
    ...
    result = runner.invoke(cli.app, ["db", "drop-db", "--db-name", "course_db_01", "--yes"])
...
def test_a_dry_run_needs_no_confirmation_because_it_changes_nothing(runner, server):
    ...
    result = runner.invoke(cli.app, ["--dry-run", "db", "drop-db", "--db-name", "course_db_01"])
...
def test_dropping_a_user_is_gated_too(runner, server):
    ...
    result = runner.invoke(cli.app, ["db", "drop-user", "--user-name", "course_user_01"], input="n\n")
...
def test_show_dbs_prints_what_the_manager_found(runner, server):
    ...
    result = runner.invoke(cli.app, ["db", "show-dbs"])
...
def test_the_readonly_command_is_only_reachable_under_its_new_name(runner):
    assert runner.invoke(cli.app, ["db", "create-shared-user-readonly", "--help"]).exit_code == 0
    # Renamed in 0.6.0 with no alias.
    assert runner.invoke(cli.app, ["db", "create-user-readonly", "--help"]).exit_code != 0


def test_set_user_password_takes_the_password_without_prompting_for_it(runner, server):
    result = runner.invoke(
        cli.app,
        ["db", "set-user-password", "--user-name", "course_user_01", "--password", "n3w-secret"],
    )
