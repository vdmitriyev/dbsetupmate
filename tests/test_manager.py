...
def test_next_names_start_at_one_on_an_empty_server(mate):
    names = mate.next_database_names()

    assert (names.database, names.user, names.order) == ("course_db_01", "course_user_01", 1)


def test_next_names_continue_after_the_highest_number(mate, server):
...
def test_next_names_ignore_unrelated_databases(mate, server):
    server.databases = ["course_db_01", "course_db_backup", "postgres"]

    assert mate.next_database_names().order == 2


def test_next_names_query_is_anchored_and_parameterised(mate, server):
    mate.next_database_names()
...
def test_create_database_runs_the_expected_statements_in_order(mate, server):
...
def test_create_database_returns_what_it_created(mate):
...
def test_identifiers_are_quoted_rather_than_interpolated(mate, server):
    ...
    create_database = [item for item in server.executed if "CREATE DATABASE" in item.text][0]
    ...
    assert create_database.statement == expected
...
def test_create_database_uses_autocommit_for_create_database(mate, server):
    mate.create_database("course_db_01", "course_user_01", "s3cret")
...
def test_demo_access_can_be_skipped(mate, server):
    created = mate.create_database("course_db_01", "course_user_01", "s3cret", grant_demo_access=False)
...
def test_a_failed_create_database_rolls_the_roles_back(mate, server):
...
def test_an_unusable_name_is_rejected_without_connecting(mate, server, name):
    with pytest.raises(InvalidIdentifierException):
        mate.create_database(name, "course_user_01", "s3cret")
...
def test_an_empty_password_is_rejected_without_connecting(mate, server, password):
    with pytest.raises(DBMateException, match="password is required"):
        mate.create_database("course_db_01", "course_user_01", password)
...
def test_a_dry_run_executes_no_ddl(config, server):
    created = PostgresMate(config, dry_run=True).create_database("course_db_01", "course_user_01", "s3cret")
...
def test_a_dry_run_still_runs_the_read_only_checks(config, server):
    ...
    with pytest.raises(DatabaseAlreadyExistsException):
        PostgresMate(config, dry_run=True).create_database("course_db_01", "course_user_01", "s3cret")
...
def test_a_dry_run_readonly_user_executes_no_ddl(config, server):
    PostgresMate(config, dry_run=True).create_readonly_user()
...
def test_create_readonly_user_defaults_to_the_configured_role(mate, server):
    created_user = mate.create_readonly_user()
...
def test_create_readonly_user_grants_connect_and_select(mate, server):
    mate.create_readonly_user("reader", "reader-secret")
...
def test_create_readonly_user_does_not_harden_the_demo_schema(mate, server):
...
def test_init_demo_database_does_not_grant_the_demo_database_to_itself(mate, server):
    created = mate.init_demo_database()
...
def test_init_demo_database_never_connects_as_the_demo_user_before_it_exists(mate, server):
